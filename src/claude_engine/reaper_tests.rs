...
fn enabled_reads_env() {
    assert!(with_env("OPMAN_CLAUDE_REAP", None, enabled), "default enabled");
...
fn ttl_ms_reads_env() {
    ...
    assert_eq!(with_env("OPMAN_CLAUDE_AGENT_TTL_SECS", Some("120"), ttl_ms), 120_000);
    // Zero is filtered → falls back to default.
    assert_eq!(with_env("OPMAN_CLAUDE_AGENT_TTL_SECS", Some("0"), ttl_ms), DEFAULT_TTL_SECS * 1000);
    // Unparseable → default.
    assert_eq!(with_env("OPMAN_CLAUDE_AGENT_TTL_SECS", Some("nope"), ttl_ms), DEFAULT_TTL_SECS * 1000);
}
...
fn clear_short_ids_drops_ids_and_ignores_empty() {
    ...
    e.clear_short_ids(&[]);
    assert_eq!(e.get_session("ses").unwrap().short_id.as_deref(), Some("aa"));
...
async fn reap_once_reaps_stale_current_target_and_clears_short_id() {
    ...
    assert!(e.get_session("ses").unwrap().short_id.is_none(), "short_id cleared");
}
