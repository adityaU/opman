...
impl App {
    ...
    pub(crate) async fn setup_subagent_slack_thread(
        ...
        let subagent_ts =
            match crate::slack::post_message(&client, &bot_token, &parent_channel, &top_msg, None)
                .await
            {
                Ok(ts) => ts,
                Err(e) => {
...

        {
            ...
            s.thread_sessions
                .insert(subagent_ts.clone(), (project_idx, child_sid.clone()));
    ...
    pub(crate) fn handle_sse_message_updated(
        ...
        if let Some(ss) = self.slack_state.clone() {
            ...
            tokio::spawn(async move {
                ss.lock().await.notify_relay(&sid);
            });
        }
    }
        ...
        if let (Some(ref ss), Some(ref auth)) = (self.slack_state.clone(), self.slack_auth.clone())
        {
            ...
            tokio::spawn(async move {
                ...
                if let Some(ref msg_ts) = existing_ts {
                    if let Err(e) =
                        crate::slack::update_message(&client, &bot_token, &channel, msg_ts, &text)
                            .await
                    {
                        tracing::warn!(
    ...
    pub(crate) fn handle_sse_permission_asked(
        &mut self,
        project_idx: usize,
    ...
    pub(crate) fn handle_sse_question_asked(
        &mut self,
        project_idx: usize,
