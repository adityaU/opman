...
fn build_inner_loads_all_persisted_collections() {
    ...
    db.insert_delegated_work(&DelegatedWorkItem {
        id: "d1".into(),
        title: "T".into(),
        assignee: "a".into(),
        scope: "s".into(),
    ...
    assert!(inner.missions.contains_key("m1"));
    assert!(inner.personal_memory.contains_key("mem1"));
    assert!(inner.routines.contains_key("r1"));
    assert_eq!(inner.routine_runs.len(), 1);
    assert!(inner.delegated_work.contains_key("d1"));
    assert!(inner.workspaces.contains_key("ws1"));
