...
fn list_sorted_by_updated_desc() {
    ...
    assert_eq!(list.iter().map(|r| r.id.clone()).collect::<Vec<_>>(), vec!["b", "c", "a"]);
}
...
fn routine_run_ordering_desc() {
    let db = Db::open_memory().unwrap();
    for (id, created) in [("r1", "2025-01-01"), ("r2", "2025-01-03"), ("r3", "2025-01-02")] {
    ...
    assert_eq!(runs.iter().map(|r| r.id.clone()).collect::<Vec<_>>(), vec!["r2", "r3", "r1"]);
}
...
fn target_mode_conversions_roundtrip_and_unknown() {
    for m in [RoutineTargetMode::ExistingSession, RoutineTargetMode::NewSession] {
