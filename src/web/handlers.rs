...
/// GET /api/agents — list available agents from the project's opencode config.
///
/// Reads the project's `opencode.json` (or `.opencode/config.json`) to extract
/// agent definitions. Falls back to the default "coder" + "task" agents.
pub async fn get_agents(
    ...
    // Try reading opencode config from known locations
    let config_paths = [
    ...
    for config_path in &config_paths {
        if let Ok(content) = tokio::fs::read_to_string(config_path).await {
            if let Ok(json) = serde_json::from_str::<serde_json::Value>(&content) {
                // opencode config has an "agents" field: Record<string, AgentConfig>
    ...
    // Always ensure "coder" and "task" are present (they're built-in)
    let has_coder = agents.iter().any(|a| a.id == "coder");
    let has_task = agents.iter().any(|a| a.id == "task");
    ...
    if !has_task {
        agents.push(AgentEntry {
            ...
            description: "Autonomous task agent".to_string(),
        });
    }

    // Sort: coder first, task second, rest alphabetical
    agents.sort_by(|a, b| {
