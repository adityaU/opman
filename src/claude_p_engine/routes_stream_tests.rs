...
async fn event_stream_emits_connected_then_matched_event() {
    let e = engine();
    let resp = event_stream(State(e.clone()), headers("/proj")).await.into_response();
    ...
    e.emit("/proj", "message.updated", serde_json::json!({ "info": { "id": "m1" } }));
...
async fn event_stream_empty_dir_receives_all_events() {
    ...
    let resp = event_stream(State(e.clone()), headers("")).await.into_response();
    let mut body = resp.into_body().into_data_stream();
    let _connected = next_frame(&mut body).await;

    e.emit("/whatever", "session.status", serde_json::json!({ "sessionID": "x" }));
...
async fn event_stream_filters_out_other_dir_events() {
    ...
    e.emit("/other", "message.updated", serde_json::json!({ "info": { "id": "m2" } }));
