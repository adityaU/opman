...
pub fn handle_key_event(app: &mut App, key: KeyEvent) -> Result<()> {
    ...
    if app.watcher_modal.is_some() {
        return handle_watcher_modal_keys(app, key);
    }
...
fn open_watcher_modal(app: &mut App) {
    use crate::app::{WatcherField, WatcherModalState, WatcherSessionEntry};
...
fn fetch_watcher_session_messages(app: &App, session_id: &str, project_idx: usize) {
...
fn handle_watcher_modal_keys(app: &mut App, key: KeyEvent) -> Result<()> {
    ...
    match active_field {
        WatcherField::SessionList => {
            handle_watcher_session_list_keys(app, key)?;
        }
        WatcherField::Message => {
            handle_watcher_message_keys(app, key)?;
        }
        ...
        WatcherField::OriginalMessageList => {
            handle_watcher_orig_message_keys(app, key)?;
        }
...
fn handle_watcher_session_list_keys(app: &mut App, key: KeyEvent) -> Result<()> {
    match key.code {
        KeyCode::Up | KeyCode::Char('k') => {
            if let Some(ref mut m) = app.watcher_modal {
                if m.selected_session_idx > 0 {
                    ...
                    fetch_watcher_session_messages(app, &sid, pidx);
                }
            }
        }
        KeyCode::Down | KeyCode::Char('j') => {
...
fn handle_watcher_message_keys(app: &mut App, key: KeyEvent) -> Result<()> {
    match key.code {
        KeyCode::Char('d') if key.modifiers.contains(KeyModifiers::CONTROL) => {
...
fn handle_watcher_orig_message_keys(app: &mut App, key: KeyEvent) -> Result<()> {
    match key.code {
        KeyCode::Up | KeyCode::Char('k') => {
            if let Some(ref mut m) = app.watcher_modal {
...
fn handle_watcher_timeout_keys(app: &mut App, key: KeyEvent) -> Result<()> {
    match key.code {
