...
export const MOCK_MESSAGES = [
  {
    ...
    parts: [{ type: "text", id: "part_001", messageID: "msg_001", text: "Hello, how are you?" }],
    metadata: null,
  },
  {
    info: {
 ...
 * so the Leptos app skips the login screen.
 */
export async function setupMockAPI(page: Page) {
  // Catch-all: prevent any unmocked /api/* request from hitting the real
  // backend. Playwright checks routes in REVERSE registration order, so this
  // is registered first (lowest priority).
  await page.route(
  ...
  await page.route(`**/api/session/${SESSION_ID}/messages*`, (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ messages: MOCK_MESSAGES, has_more: false, total: MOCK_MESSAGES.length }),
    })
  );
  ...
  await page.route("**/api/themes", (route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify([{ id: "glassy", name: "Glassy" }, { id: "flat", name: "Flat" }]),
    })
  );
  ...
  await page.route("**/api/pending", (route) =>
    route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ permissions: [], questions: [] }) })
  );
...
/**
 * Navigate to the Leptos UI (/ui) and wait for the chat layout.
 *
 * The Leptos app uses cookie-based auth (credentials: same-origin), not
 * sessionStorage tokens. By mocking /api/auth/verify to return 200 and
 * /api/public/bootstrap to return valid data, the WASM app transitions
 * directly to the authenticated ChatLayout.
 */
export async function navigateAuthenticated(page: Page) {
  await setupMockAPI(page);

  // Navigate to the Leptos UI — mocked auth/verify returning 200 means
  // the app treats us as authenticated (cookie-based auth).
  await page.goto("/ui");

  // Wait for the chat layout to be visible (WASM load + auth check + render)
