        ...
        <div className="memory-create">
          ...
          <div className="memory-create-footer">
            ...
            <button
              ...
              disabled={saving || !label.trim() || !content.trim() || (scope === "session" && !activeSessionId)}
            >
              <Plus size={14} />
                        ...
                        {editingId === item.id ? (
                          <>
                            ...
                            <select
                              className="memory-select"
                              value={editingScope}
                              onChange={(e) => setEditingScope(e.target.value as MemoryScope)}
                            >
                              {SCOPE_OPTIONS.map((option) => (
                                <option key={option} value={option}>
                                  {formatScope(option)}
                        ...
                        {editingId === item.id ? (
                          <>
                            ...
                            <button
                              ...
                              onClick={cancelEdit}
                              aria-label={`Cancel edit for ${item.label}`}
                            >
                              <X size={14} />
                            </button>
                          </>
...
function describeScope(item: PersonalMemoryItem, projects: ProjectInfo[]): string {
