...
export interface FileExplorerState extends FileActionsState {
  // Directory browsing
  currentPath: string;
  entries: FileEntry[];
  loadingDir: boolean;
...
export function useFileExplorer(
  ...
  const loadFile = useCallback(async (path: string, line?: number | null) => {
    ...
    try {
      ...
      if (isDocRenderType(renderType)) {
        const resp = await docRead(path);
        docData = resp.data;
      } else if (renderType !== "image" && renderType !== "audio" && renderType !== "video" && renderType !== "pdf" && renderType !== "model3d" && renderType !== "binary") {
  ...
  // ── File management actions (extracted hook) ──────────

  const actions = useFileActions({
    currentPath, expandedDirs, openFiles, activeFilePath,
    loadDirectory, closeFile,
    setEntries, setCurrentPath, setDirChildren, setExpandedDirs,
    setOpenFiles, setActiveFilePath, setSaveStatus, onError,
  });
